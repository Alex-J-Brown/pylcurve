...
def plot_LC_GP(model, params, show=True, save=False, name='LC.pdf', dataname=None):
    ...
    for idx in range(n_cols*2):
        idx_half = int(idx/2)
        band = list(model.lightcurves.keys())[idx_half]
        t, ym, gp_mean, gp_var, y, ye = model.model_GP(band, params)
        if dataname:
            np.savetxt('{}_{}.dat'.format(dataname,band), np.column_stack((t,ym)))
        ...
        if idx % 2 == 0:
            ...
            ax.errorbar(phase, (y - (ym+gp_mean)) / np.std((y - (ym+gp_mean))),
                        yerr=ye / np.std((y - (ym+gp_mean))), lw=0, elinewidth=1,
                        marker='.', ms=2, zorder=1, color=band_colours[band])
            ax.axhline(0, c='k', ls='--', zorder=2)
            ax.tick_params(top=True, bottom=True, left=True, right=True, direction='in')
            yabs_max_res = np.max(np.abs(ax.get_ylim()))
...
def plot_CP(fchain, namelist, composition='CO', name='CP.pdf', **kwargs):
    ...
    if composition=='He':
        ...
        idx_m1 = namelist.index('m1')
        ndim = len(namelist)
        axes = np.array(fig.axes).reshape((ndim, ndim))
        axes[idx_m1, idx_t1].fill_between(x=teff_lim, y1=mass_lim, color='r', alpha=0.3)
    fig.savefig(name)
    plt.close()
...
def plot_CP_reduced(fchain, namelist, composition='CO', name='CP.pdf', plotted_params = ['t1', 't2_i', 'm1', 'm2', 'incl', 'parallax'], **kwargs):
    ...
    labels = [label_dict[name] if name in label_dict else name for name in plotted_params]
    fchain_plot = fchain.copy()
    if 't0' in namelist:
        idx_t0 = namelist.index('t0')
        fchain_plot[:, idx_t0] = (fchain_plot[:,idx_t0] - np.median(fchain_plot[:,idx_t0])) * 1e5
    ...
    fig = triangle.corner(fchain_plot_cut, hist_bin_factor=2, quantiles=[0.16, 0.5, 0.84], show_titles=True,
                          title_kwargs={"fontsize": 14}, labels=labels, label_kwargs={'fontsize': 16}, **kwargs)
    ndim = len(plotted_params)
    axes = np.array(fig.axes).reshape((ndim, ndim))
    for ax in axes.ravel():
        ax.tick_params(top=True, bottom=True, left=True, right=True, direction='in', which='both')
    if composition=='He':
        idx_t1 = plotted_params.index('t1')
        idx_m1 = plotted_params.index('m1')
        ndim = len(plotted_params)
        axes = np.array(fig.axes).reshape((ndim, ndim))
        axes[idx_m1, idx_t1].fill_between(x=teff_lim, y1=mass_lim, color='r', alpha=0.3)
    fig.savefig(name, pad_inches=0.05, bbox_inches='tight')
    plt.close()
